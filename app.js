...
app.post("/api/addAnimal", function(req, res) {
  ...
  newAnimal.routeName = newAnimal.name.replace(/\s+/g, "").toLowerCase();
  characters.push(newAnimal);
...
app.get("/api/getAnimal/:animal", function(req, res) {
  ...
  for (var i = 0; i < animals.length; i++) {
    if (chosen === animals[i].animalType) {
      res.render('pages/any', {title: req.params.animal, animal: animals[i]})
    }
  }

  res.render('pages/any', {title: 'Not Found', animal: blank})
